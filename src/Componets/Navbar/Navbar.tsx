    ...
    <nav className="fixed top-0 w-full z-50 bg-gradient-to-r from-white/70 via-white/60 to-white/70 backdrop-blur-xl shadow-lg border-b border-gray-200">
      <div className="max-w-7xl mx-auto flex justify-between items-center py-3 px-4 sm:px-6 lg:px-8">
        ...
        <Link to="/" className="group" onClick={() => setActiveLink("/")}>
          <span className="text-xl sm:text-2xl font-extrabold tracking-wide text-gray-900">
            Sazzadur{" "}
        ...
        <ul className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <li
              key={link.path}
              className={`relative group text-base xl:text-lg font-semibold text-gray-700 hover:text-red-600 transition duration-300 cursor-pointer
                ${
                  activeLink === link.path
                    ? "bg-gradient-to-r from-pink-100/50 to-red-100/50 rounded-full px-2 py-1"
                    : ""
                }`}
            >
              <HashLink
          ...
          <li>
            <HashLink
              ...
              className="ml-2 px-4 py-2 rounded-full bg-gradient-to-r from-pink-500 to-red-500 text-white font-semibold shadow-md hover:shadow-lg transform hover:scale-105 transition duration-300 text-sm lg:text-base"
            >
              Hire Me
            </HashLink>
          </li>
        ...
        <div className="lg:hidden absolute top-16 left-0 w-full bg-white/95 backdrop-blur-xl shadow-lg border-t border-gray-200 animate-slide-down">
          <ul className="flex flex-col items-center gap-4 py-6 px-4">
            {navLinks.map((link) => (
              <li
                key={link.path}
                className={`text-base sm:text-lg font-semibold text-gray-700 hover:text-red-600 transition-colors duration-300 w-full text-center
                  ${
                    activeLink === link.path
            ...
            <li>
              <HashLink
                ...
                className="px-6 py-2 rounded-full bg-gradient-to-r from-pink-500 to-red-500 text-white font-semibold shadow-md hover:shadow-lg transform hover:scale-105 transition duration-300 text-base sm:text-lg"
              >
                Hire Me
              </HashLink>
            </li>
